...
import type { AmbientPerfTier } from "./components/ambient/ambient-builder.js";
import {
...
} from "./components/ambient/mob-sector-debug.js";
...
export const bootstrapGame = (
  ...
  let ambientPerfTier: AmbientPerfTier = "high";
  let perfSampleAccumMs = 0;
  ...
  /** Brief snap-in on the ball as it leaves the tee — opens the cinematic
   *  flight curve before the camera pulls back. Kept gentle (well under the
   *  earlier 1.18) so the open shot eases rather than punches. */
  const FLIGHT_CAM_BALL_TIGHT_MUL = 1.08;
  ...
  /** Pre-shot windup shake: matches `SWING_WINDUP_MS` in round.ts so amplitude peaks right at strike. */
  const SWING_SHAKE_WINDUP_MS = 1300;
  const SWING_SHAKE_PEAK_PX = 1.4;
  const SWING_SHAKE_FREQ_HZ = 12;
  /** Lingering shake after the strike: brief residual rumble while the ball leaves. */
  const SWING_SHAKE_TAIL_MS = 120;
  /** Slow, low-amplitude scale pulse layered on top of the shake — reads as a
   *  steady "heartbeat" tension swell during the windup rather than a rattle. */
  const SWING_PULSE_AMPLITUDE = 0.0035;
  const SWING_PULSE_FREQ_HZ = 1.4;
  /** Sustained zoom-IN toward the character through the windup — punches in
   *  on the swing, then releases when the strike fires the flight curve. */
  const SWING_CHAR_ZOOM_PEAK = 1.06;
  let swingCharZoomCurrent = 1;
  let impactZoomTarget = 1;
  let impactZoomCurrent = 1;
  let impactZoomResetAt = 0;
  let impactPulseStartedAt = 0;
  let preShotStartedAtMs = 0;
  let appliedShakeX = 0;
  let appliedShakeY = 0;
  ...
  const objectLayersForScale = (_scale = getReferenceScale()): ObjectLayers => {
  ...
  let prevBallFramePos = { x: BALL_START_X, y: BALL_START_Y };
  let currBallFramePos = { x: BALL_START_X, y: BALL_START_Y };
  ...
  const registerAmbientMotion = (
    ...
    ambientMotions.push({
      ...
      clampYMax: band.maxY,
      prevX: node.x,
      prevY: isGroundCart ? groundedY : node.y,
      currentRotation: 0,
    });
  };
  ...
  const updateCamera = (dt: number, nowMs: number): void => {
    ...
    /** Slow zoom smoothing so the cinematic curve glides instead of snapping
     *  whenever the flight target steps a lot in one frame (e.g. at strike). */
    const zoomAlpha =
      1 - Math.exp(-(game.phase === "flight" ? 2.4 : 3.5) * dtCap);

    let flightZoomGoal = 1;
    if (game.phase === "flight") {
      const duration = getFlightDurationSec(currentPlan?.crashAtSec ?? 2);
      const t0 = game.flightStartedAtMs > 0 ? game.flightStartedAtMs : nowMs;
      const elapsed = Math.max(0, (nowMs - t0) / 1000);
      const p = Math.min(1, duration > 1e-6 ? elapsed / duration : 0);
      /**
       * Cinematic three-phase flight zoom — shaped to glide, not punch:
       *   [0, openIn]      — slow push IN on the ball as it leaves the tee.
       *   [openIn, widePeak] — long pull-back away from the ball (crane shot).
       *   [widePeak, 1]    — gradual close-in as the ball approaches landing.
       * Wider middle segment + smoothstep joints keeps it on the rails.
       */
      const openIn = 0.24;
      const widePeak = 0.7;
      const smoothstep = (s: number): number => s * s * (3 - 2 * s);
      if (p < openIn) {
        const sub = smoothstep(p / openIn);
        flightZoomGoal = 1 + (FLIGHT_CAM_BALL_TIGHT_MUL - 1) * sub;
      } else if (p < widePeak) {
        const sub = smoothstep((p - openIn) / (widePeak - openIn));
        flightZoomGoal =
          FLIGHT_CAM_BALL_TIGHT_MUL +
          (FLIGHT_CAM_WIDE_MUL - FLIGHT_CAM_BALL_TIGHT_MUL) * sub;
      } else {
        const sub = smoothstep((p - widePeak) / (1 - widePeak));
        flightZoomGoal =
          FLIGHT_CAM_WIDE_MUL +
          (FLIGHT_CAM_TIGHT_MUL - FLIGHT_CAM_WIDE_MUL) * sub;
      }
    }
    flightZoomCurrent += (flightZoomGoal - flightZoomCurrent) * zoomAlpha;

    // Sustained zoom-IN on the character during the swing windup; relaxes
    // back to 1 as soon as the phase leaves `preShot` so the flight curve
    // owns the framing once the ball is airborne.
    let swingCharZoomGoal = 1;
    if (game.phase === "preShot" && preShotStartedAtMs > 0) {
      const elapsed = Math.max(0, nowMs - preShotStartedAtMs);
      const p = Math.min(1, elapsed / SWING_SHAKE_WINDUP_MS);
      const eased = p * p * (3 - 2 * p);
      swingCharZoomGoal = 1 + (SWING_CHAR_ZOOM_PEAK - 1) * eased;
    }
    swingCharZoomCurrent +=
      (swingCharZoomGoal - swingCharZoomCurrent) * zoomAlpha;
    ...
    if (impactPulseStartedAt > 0) {
      ...
      if (
        pulseElapsed >= 0 &&
        pulseElapsed * 1000 <= IMPACT_PULSE_DURATION_MS
      ) {
    ...
    // Swing-windup heartbeat: a slow, shallow scale pulse during `preShot`
    // that ramps in with the same p² envelope as the (now reduced) shake so
    // the tension reads as a swell-and-thud rather than a rattle.
    let swingPulseMul = 1;
    if (game.phase === "preShot" && preShotStartedAtMs > 0) {
      const elapsed = Math.max(0, nowMs - preShotStartedAtMs);
      const p = Math.min(1, elapsed / SWING_SHAKE_WINDUP_MS);
      ...
      swingPulseMul =
        1 +
        SWING_PULSE_AMPLITUDE *
          env *
          Math.sin(2 * Math.PI * SWING_PULSE_FREQ_HZ * tSec);
    }

    // Strip last frame's shake offset so the controller's prev-pos lerp anchor
    // is shake-free; otherwise the offset accumulates inside its smoothing.
    world.x -= appliedShakeX;
    world.y -= appliedShakeY;
    ...
    // Pre-shot windup shake: amplitude grows as p², peaking just before strike.
    // A short tail keeps the rumble alive into the first frames of `flight`.
    let shakeAmp = 0;
    if (game.phase === "preShot" && preShotStartedAtMs > 0) {
      const elapsed = Math.max(0, nowMs - preShotStartedAtMs);
      const p = Math.min(1, elapsed / SWING_SHAKE_WINDUP_MS);
      shakeAmp = SWING_SHAKE_PEAK_PX * (p * p);
    } else if (game.phase === "flight" && preShotStartedAtMs > 0) {
      const sinceFlight = Math.max(
        0,
        nowMs - (game.flightStartedAtMs > 0 ? game.flightStartedAtMs : nowMs),
      );
      if (sinceFlight < SWING_SHAKE_TAIL_MS) {
        const decay = 1 - sinceFlight / SWING_SHAKE_TAIL_MS;
        shakeAmp = SWING_SHAKE_PEAK_PX * decay;
      }
    } else if (preShotStartedAtMs > 0 && game.phase !== "flight") {
      preShotStartedAtMs = 0;
    }
    if (shakeAmp > 0.01 && !devFitWholeWorld) {
      const tSec = nowMs / 1000;
      appliedShakeX =
        shakeAmp *
        Math.sin(2 * Math.PI * SWING_SHAKE_FREQ_HZ * tSec);
      appliedShakeY =
        shakeAmp *
        0.7 *
        Math.cos(2 * Math.PI * (SWING_SHAKE_FREQ_HZ * 1.3) * tSec + 0.7);
    } else {
      appliedShakeX = 0;
      appliedShakeY = 0;
    }
  ...
  const updateAmbientDecor = (dt: number, now: number): void => {
    ...
    for (const motion of ambientMotions) {
      ...
      // ── Compute movement-direction rotation ──────────────────────
      const dx = motion.node.x - motion.prevX;
      const dy = motion.node.y - motion.prevY;
      motion.prevX = motion.node.x;
      motion.prevY = motion.node.y;

      // Skip rotation for ground carts and clouds — they stay flat.
      if (
        motion.kind !== "cart" &&
        motion.layerId !== 0 &&
        motion.aiType !== "cloud"
        ...
        if (speed > 0.3) {
          // Angle of movement; when flipped (scale.x < 0) we mirror the angle.
          const rawAngle = Math.atan2(dy, dx);
          const facingRight = motion.node.scale.x >= 0;
          // When sprite faces right, rawAngle is the tilt directly.
          // When sprite faces left (flipped), we mirror: π - rawAngle.
          const targetRotation = facingRight ? rawAngle : Math.PI - rawAngle;
          // Clamp max tilt so sprites don't flip upside-down.
          const maxTilt = 0.05; // ~26°
          const clampedTarget = Math.max(
            -maxTilt,
            Math.min(maxTilt, targetRotation),
          );
          const rotSmoothing = 0.08;
          motion.currentRotation +=
            (clampedTarget - motion.currentRotation) * rotSmoothing;
        } else {
          // No significant movement — ease back to level.
          motion.currentRotation *= 0.92;
        }
        if (Math.abs(motion.currentRotation) < 0.001)
          motion.currentRotation = 0;
        motion.node.rotation = motion.currentRotation;
      }

      // ── Horizontal flip (face direction of travel) ───────────────
      if (
  ...
  const updateBall = (now: number, dtSec: number): void => {
    ...
    if (phase === "preShot" && lastPhase !== "preShot") {
      ...
      preShotStartedAtMs = now;
    }
    if (phase === "flight" && lastPhase !== "flight") {
      impactZoomTarget = 1;
      impactZoomCurrent = 1;
      // Flight zoom starts at neutral so the slow zoom-OUT through mid-flight
      // is visible (was snapping to wide and creeping tighter).
      flightZoomCurrent = 1;
      ...
      prewarmFlightRevealAlongArc(game.isJackpot ? "holeInOne" : "crash");
    }
    ...
    if (phase === "flight") {
      ...
      if (collision && now >= collision.impactAt) {
        ...
        const multSpin = 1 + Math.log(Math.max(1, game.multiplier)) * 0.65;
        ...
        const speedFactor = 1 + Math.log(Math.max(1, game.multiplier)) * 0.95;
    ...
    if (phase === "landed") {
      ...
      if (elapsed <= arrivalDuration) {
        const x = landedFromX + (HOLE_X - landedFromX) * rollP;
        ...
        const sinkFromX = HOLE_X + Math.cos(orbitEndAngle) * holeRadiusX;
        const sinkFromY = holeY() + Math.sin(orbitEndAngle) * holeRadiusY;
        fireBallSprite.x =
  ...
  const pointToSegmentDistance = (
    ...
    if (segLenSq <= 1e-6) return Math.hypot(px - x0, py - y0);
    const t = Math.max(
      0,
      Math.min(1, ((px - x0) * dx + (py - y0) * dy) / segLenSq),
    );
    const cx = x0 + dx * t;
    const cy = y0 + dy * t;
  ...
  const canSpawnEventAtBall = (
    event: DecorativeEvent,
    target?: { x: number; y: number },
  ): boolean => {
    const pos = ballPos();
    if (target && game.phase === "flight") {
      const hitRadiusByKind: Record<DecorativeEvent["kind"], number> = {
        bird: 120,
        wind: 130,
        helicopter: 150,
        plane: 170,
      ...
      const hitRadius = hitRadiusByKind[event.kind];
  ...
  const spawnEffect = (event: DecorativeEvent, now: number): void => {
    ...
    if (!canSpawnEventAtBall(event, plannedPos)) return;
    planned.highlightUntil = now + 700;
    const pos = plannedPos;
  ...
  const animate = (ticker: Ticker): void => {
    ...
    perfSampleAccumMs += ticker.deltaMS;
    perfSampleFrames += 1;
    if (perfSampleAccumMs >= PERF_SAMPLE_WINDOW_MS) {
      const fps = (perfSampleFrames * 1000) / Math.max(1, perfSampleAccumMs);
      ambientPerfTier = fps < 46 ? "low" : fps < 56 ? "medium" : "high";
      perfSampleAccumMs = 0;
      perfSampleFrames = 0;
    }
    const dtCap = Math.min(0.05, Math.max(0, dt));
    const now = performance.now();
    prevBallFramePos = currBallFramePos;
    updateAmbientDecor(dt, now);
    updatePlannedHazards(now);
    updateCollision(now);
    updateBall(now, dtCap);
    currBallFramePos = ballPos();
    updateBallFx(now);
    refreshVisualMode();
    updateCharacter(now);
    ...
    impactZoomCurrent += (impactZoomTarget - impactZoomCurrent) * impactAlpha;
    updateCamera(dt, now);
    updateOverlay(now);
    if (devMobSectorDebug && mobSectorDebugG && mobSectorDebugLabelRoot) {
      const lyr = objectLayersForScale(getReferenceScale());
      const span = getAmbientSpawnXSpan(WORLD_W, canvasW);
      redrawMobSpawnSectorDebug(
        mobSectorDebugG,
        WORLD_W,
        WORLD_H,
  ...
  const buildScene = (): void => {
    ...
    const cartForbiddenIntervals: Array<readonly [number, number]> = [
      ...terrainHazardZones.map(
        (zone) => [zone.startX - 80, zone.endX + 80] as const,
      ),
      ...mapLayout.features
        .filter((f) => f.type === "water" || f.type === "sand")
        .map(
          (f) =>
            [
              (f.leftEdgeX ?? f.x - 140) - 80,
              (f.rightEdgeX ?? f.x + 140) + 80,
            ] as const,
        ),
    ];
    const ambientPack = buildObjectLayerSystem(
      ...
      objectLayersForScale(getReferenceScale()),
      {
        ...
        cartForbiddenIntervals,
        cartMinX: BALL_START_X + 320,
        cartMaxX: PLAY_END_X - 250,
      },
    );
  ...
  const init = async (): Promise<void> => {
    ...
    await Assets.load(GAME_CORE_ASSET_MANIFEST.map(resolveAssetSrc), (p) =>
      hooks.onProgress?.(0.05 + p * 0.78),
    );
    ...
    unsubWaterSurfaceLoss = onWaterSurfaceLoss(() => {
      ...
      const pos = trajectoryPoint(1, game.isJackpot ? "holeInOne" : "crash");
      fireBallSprite.x = pos.x;
      fireBallSprite.y = pos.y;

 ...
 *   - sand:  a continuous stretch (300–600 px) where the surface is nearly
 *            flat (max - min < `FLAT_TOLERANCE`). Sand stretches that overlap
 *            an already-claimed water zone are dropped.
 *
...
export const analyzeTerrainForHazards = (
	...
	for (let leftIdx = 0; leftIdx < sampleCount; leftIdx += 2) {
		...
		if (bestRight >= 0) {
			...
			// Y grows downward, so the LOWER-elevation lip has the LARGER Y.
			// Water level sits a few px below that lip (i.e. larger Y) so it
			// can never overflow either edge.
	...
	// Fallback: strict hill-in-bowl rejection can legitimately remove all
	// water zones on some terrain seeds. Keep visuals alive by selecting the
	// single deepest valley candidate when no water was found.
	if (!zones.some((z) => z.type === "water")) {
		let bestFallback:
			| { startX: number; endX: number; topY: number; depth: number }
			| null = null;
		for (let leftIdx = 0; leftIdx < sampleCount; leftIdx += 2) {
			const leftY = samples[leftIdx]!;
			for (
				...
				if (!bestFallback || depth > bestFallback.depth) {
					bestFallback = {
						startX,
						endX,
		...
		if (bestFallback) {
			...
			claimed.push({ start: bestFallback.startX, end: bestFallback.endX });
		}
	}

	const FLAT_MIN_WIDTH = 300;
	const FLAT_MAX_WIDTH = 600;
	const FLAT_TOLERANCE = 9;
...
export const buildProceduralFrontTerrain = ({
	...
	const hazardZones = analyzeTerrainForHazards(worldW, hillSurfaceY);
	const waterZones = hazardZones.filter((z) => z.type === "water");
	// Physics path: ball follows ground inside water zones.
	const getEffectiveSurfaceY = (x: number): number => {
		for (const zone of waterZones) {
			if (x >= zone.startX && x <= zone.endX) {
				return hillSurfaceY(x);
			}
		}
		return hillSurfaceY(x);
	};
	...
	if (SHOW_ROUTE_DEBUG) {
		routeLine.moveTo(0, getEffectiveSurfaceY(0) - frontTerrain.y);
		for (let x = 1; x <= worldW; x += 1) {
			routeLine.lineTo(x, getEffectiveSurfaceY(x) - frontTerrain.y);
		}
	...
	for (const [index, zone] of hazardZones.entries()) {
		...
		if (isWater) {
			...
			sprite.height = depth + 25;
			sprite.zIndex = 5;
		} else {
			sprite.y = localY(hillSurfaceY(centerX)) - 2;
			sprite.width = width + 30;
			sprite.height = 45;
			sprite.zIndex = 4;
		}
	...
	groundFill
		.rect(-1000, frontTargetH - 20, worldW + 2000, 4200)
		.fill(0x182028);

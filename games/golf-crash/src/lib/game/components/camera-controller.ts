...
export const computeScale = (
  ...
  const visibleGroundHeight = aspect > 1 ? 1500 : 1100;
  const visibleFlightHeight = aspect > 1 ? 2200 : 2800;
...
export const updateCamera = ({
  ...
  const followRest = phase === "idle" || phase === "cashOut" || phase === "lose";
  const shouldFollow =
  ...
  const focusY = ballFollow ? canvasH * dynamicFlightFocusY : canvasH * idleFocusY;
  const centeredCamY = focusY - targetY * scale - (ballFollow ? bottomUiPaddingPx : 0);
